...
一个面向 macOS 的安全型、仅做归档的工具，用来通过清理 stale subagent 线程，恢复 Codex 的 cluster 协作容量。

English version: [README.md](README.md)

## 它解决什么痛点

当本地状态库里残留过多 stale subagent 线程时，Codex 往往会在创建新 subagents 时变得过于保守，导致 cluster 能力实际下降。

这个工具适合想要：

- 恢复可用的 cluster 容量
- 清理 stale subagent 堆积
- 先安全预演、再决定是否执行
...
1. 下载本仓库。
2. 双击 `bin/codex-cluster-unlocker.command`。
